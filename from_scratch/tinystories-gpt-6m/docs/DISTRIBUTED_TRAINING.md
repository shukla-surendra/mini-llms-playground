...
`train_ddp.py` and `train_fsdp.py` wrap the exact same causal-LM model and data
`train.py` already trains — same `model.py`, same tokenized data — adding only the
...
tested-working launch path — `python train_ddp.py` directly, no external launcher — using
...
if "RANK" in os.environ:
    main(int(os.environ["RANK"]), int(os.environ["WORLD_SIZE"]))  # torchrun path
else:
    mp.spawn(main, args=(WORLD_SIZE,), nprocs=WORLD_SIZE, join=True)  # this project's default
```
...
expects from an accelerator backend. The fix, in `train_fsdp.py`:
...
- **No checkpoint saving for FSDP.** `train_fsdp.py` intentionally skips checkpointing —
...
The loss trajectory above is *slower* in wall-clock terms per step than `train.py`'s own
