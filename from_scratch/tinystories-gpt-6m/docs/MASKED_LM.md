...
`model_mlm.py` and `train_mlm.py` implement BERT-style masked language modeling — a
different pretraining objective from the causal (next-token) LM the rest of this project
trains (`model.py`/`train.py`). If the concept itself (what makes an objective
...
`MaskedLMTinyStories` reuses `model.py`'s `GPTBlock` unchanged, just constructed with
`causal=False` — same block, same math, the causal mask simply isn't applied.

## The masking policy: BERT's 80/10/10 rule, exactly

For each token position, independently: with probability `MASK_PROB` (default 0.15) it's
...
See `apply_bert_masking` in [`model_mlm.py`](../model_mlm.py) for the exact implementation.
...
target (see [`model_mlm.py`](../model_mlm.py)'s module docstring for why this also means
...
`STEPS=1200`, default `BATCH_SIZE=32`/`CONTEXT_LENGTH=256`/`MASK_PROB=0.15`, same
