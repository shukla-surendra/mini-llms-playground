...
Companion to [`TRAINING.md`](TRAINING.md). While `train.py` runs, it prints a live,
...
| `250/1000` | **Current step / total steps** — `250` is how many training steps have run so far; `1000` is `STEPS` from [`TRAINING.md`'s hyperparameter table](TRAINING.md#hyperparameters-and-the-reasoning-behind-each) (the total this run was configured for) |
...
These come from `train.py`'s own code, not from `tqdm` itself:
...
near-zero during the first ~2% of steps, then smoothly decrease toward `MIN_LR` for the
...
`EVAL_INTERVAL` steps (default 250 — notice this line is *exactly* at step 250, the first
evaluation point after the initial one at step 0) and averages the loss over
`EVAL_BATCHES` separate batches for both the training set and a held-out validation set.
...
`EVAL_INTERVAL` steps — the line shown above will display the *same* `train=2.395,
...
to [`../logs/train_eval_history.csv`](../logs/train_eval_history.csv) — the full,
