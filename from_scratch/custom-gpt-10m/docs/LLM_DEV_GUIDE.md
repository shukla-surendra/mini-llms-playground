...
- `checkpoints/<label>/latest.pt` for periodic resume saves
- `checkpoints/<label>/best.pt` for best validation checkpoint
- `checkpoints/<label>/serving.pt` used for serving/inference
- `checkpoints/<label>/final.pt` saved at end of training run

Safe stop and resume:
- Press `Ctrl+C` during training to stop.
- Training catches the interrupt and saves `checkpoints/<label>/latest.pt`.
- Run `make train` again to resume from the saved step.
- Run with `RESUME_TRAINING=0 make train` to start a fresh run.
...
- Read `gpt-data`, `gpt-train`, `inference.py` line by line
...
```bash
EXTRA_DATASET='HuggingFaceH4/ultrachat_200k' EXTRA_SPLIT='train_sft' make data
```
